...
logger = logging.getLogger("Mock SMBus")
logger.warning("Using fake SMBus, could not import real library.")
...
class SMBus(object):
    ...
    def _register_fake_device(self, device):
        ...
        if device.addr in self._devices.keys():
            raise ValueError(f"Device using addr: {device.addr} already registered.")
